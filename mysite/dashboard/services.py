...
def get_product_by_order(id):
    with closing(connection.cursor()) as cursor:
        cursor.execute(""" SELECT food_product.*, food_orderproduct.count,food_orderproduct.price,
        food_orderproduct.created_at, food_product.title from food_product
         INNER JOIN food_orderproduct ON food_product.id=food_orderproduct.id  where food_orderproduct.order_id=%s""",[id])
